...
from utils.errors import MissingDescriptionError,GitHubApiKeyNotFoundError
...
class GitHubProjectLoader:
    def __init__(self,github_api_key:str=None, folder_to_clone: Path = Path("Github")):
        ...
        elif os.getenv("GITHUB_TOKEN", None):
            self.api_key = os.getenv("GITHUB_TOKEN", None)
        else:
            raise GitHubApiKeyNotFoundError("Please provide a login token or set the GITHUB_TOKEN environment variable.")
